...
bool assign(int32_t &destination, uint32_t &present, uint32_t field,
  ...
  const Scaled scaled = scaleInRange(reading.value, unit, minimum, maximum);
  if (!scaled.ok) {
    return false;
  }
...
Telemetry buildTelemetry(const SampleContext &context, const SensorSnapshot &sensors) {
  ...
  const bool temp_in_ok =
      assign(telemetry.temp_in_c, telemetry.present, Field::temp_in_c, sensors.temp_in,
             Scale::Temperature, kSht30MinTempC, kSht30MaxTempC);
  const bool rh_in_ok =
      assign(telemetry.rh_in_pct, telemetry.present, Field::rh_in_pct, sensors.rh_in,
             Scale::Humidity, kSht30MinRhPct, kSht30MaxRhPct);
  const bool temp_out_ok =
      assign(telemetry.temp_out_c, telemetry.present, Field::temp_out_c, sensors.temp_out,
             Scale::Temperature, kSht30MinTempC, kSht30MaxTempC);
  const bool rh_out_ok =
      assign(telemetry.rh_out_pct, telemetry.present, Field::rh_out_pct, sensors.rh_out,
             Scale::Humidity, kSht30MinRhPct, kSht30MaxRhPct);
  const bool weight_ok =
      assign(telemetry.weight_kg, telemetry.present, Field::weight_kg, sensors.weight,
             Scale::Weight, kMinWeightKg, kMaxWeightKg);
  const bool battery_ok =
      assign(telemetry.vbat_v, telemetry.present, Field::vbat_v, sensors.battery,
             Scale::Voltage, kMinVoltageV, kMaxVoltageV);
