...
using meliponet::Escalado;
using meliponet::testdata::kQuantidadeDeVetoresDeEscala;
using meliponet::testdata::kVetoresDeEscala;

namespace {

void test_vetores_de_escala(void) {
  for (size_t i = 0; i < kQuantidadeDeVetoresDeEscala; ++i) {
    const auto &vetor = kVetoresDeEscala[i];
    const Escalado resultado = meliponet::escalar(vetor.leitura, vetor.casas);

    TEST_ASSERT_TRUE_MESSAGE(resultado.ok, vetor.campo);
    TEST_ASSERT_EQUAL_INT32_MESSAGE(vetor.esperado, resultado.valor, vetor.campo);
  }
}
...
void test_valores_nao_finitos_sao_recusados(void) {
  TEST_ASSERT_FALSE(meliponet::escalar(NAN, 2).ok);
  TEST_ASSERT_FALSE(meliponet::escalar(INFINITY, 2).ok);
  TEST_ASSERT_FALSE(meliponet::escalar(-INFINITY, 3).ok);
}
...
void test_estouro_de_faixa_e_recusado(void) {
  TEST_ASSERT_FALSE(meliponet::escalar(1e12, 3).ok);
  TEST_ASSERT_FALSE(meliponet::escalar(-1e12, 3).ok);
}

void test_faixa_do_sensor(void) {
  // Dentro da faixa do SHT30.
  TEST_ASSERT_TRUE(meliponet::escalarNaFaixa(30.0, meliponet::kTemperatura).ok);
  // Fora: nenhum SHT30 são produz isso.
  TEST_ASSERT_FALSE(meliponet::escalarNaFaixa(120.0, meliponet::kTemperatura).ok);
  TEST_ASSERT_FALSE(meliponet::escalarNaFaixa(-60.0, meliponet::kTemperatura).ok);
  // Os limites são inclusivos.
  TEST_ASSERT_TRUE(meliponet::escalarNaFaixa(85.0, meliponet::kTemperatura).ok);
}
...
void test_peso_negativo_por_deriva_e_aceito(void) {
  const Escalado resultado = meliponet::escalarNaFaixa(-0.012, meliponet::kPeso);

  TEST_ASSERT_TRUE(resultado.ok);
  TEST_ASSERT_EQUAL_INT32(-12, resultado.valor);
}
...
void test_casas_decimais_fora_do_previsto_sao_recusadas(void) {
  TEST_ASSERT_FALSE(meliponet::escalar(30.0, -1).ok);
  TEST_ASSERT_FALSE(meliponet::escalar(30.0, 4).ok);
  TEST_ASSERT_TRUE(meliponet::escalar(30.0, 0).ok);
  TEST_ASSERT_TRUE(meliponet::escalar(30.0, 3).ok);
}
